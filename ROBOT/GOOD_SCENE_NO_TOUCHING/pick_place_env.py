...
class PickPlaceCustomEnv():
    ...
    def _get_obs(self):
        """
        "state": (13,) robot state information (6 joint angles, 3 grasp site position, 4 grasp site quaternion)
        "object": (7,) object position (x,y,z) and quaternion (qx,qy,qz,qw)
        "goal": (3,) goal position (x,y,z)
        """
        ## Robot state information
        robot_joint_angles = self.data.qpos[:6].copy().astype(np.float32) # 6 joint angles (5 arm joints + 1 jaw)
        ...
        state = np.concatenate([robot_joint_angles, grasp_site_pos, grasp_site_quat]).astype(np.float32) # (6 + 3 + 4) = 13
...
if __name__ == "__main__":
    ...
    random_object_pos = False # Randomize object position?
