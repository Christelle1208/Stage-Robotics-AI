...
class Planner():
    ...
    def _check_grasped(self, gripper_dist):
        return gripper_dist < 0.5  # Jaw joint angle near-closed (SO-ARM100 range -0.174 to 1.75)

    def _check_released(self, gripper_dist):
        return gripper_dist > 1.0  # Jaw joint angle well open
    ...
    def plan(self, obs, mjData):
        ...
        joint_angles = obs['state'][:5]  # 5 arm joint angles excluding the jaw
        gripper_dist = obs['state'][5] # Jaw angle (0.0 for closed, 1.75 for open)
        grasp_site_pos = obs['state'][6:9]  # Next 3 elements are grasp site position
        grasp_site_quat = obs['state'][9:13]  # Next 4 elements are grasp site quaternion
        ...
        if self.state != self.prev_state or \
           self.current_time - self.prev_motion_plan_time > self.motion_repanning_interval:
            ...
            self.target_gripper_dist = 1.75  # Default gripper position (open, Jaw max range)
            ...
            if self.state == "initial":
                ...
                target_joint_angles = ikresults[0][:5]
                ...
                self.target_gripper_dist = -0.174  # Fully close jaw
                ...
                target_joint_angles = ikresults[0][:5]
                self.target_gripper_dist = -0.174  # Keep jaw closed
                ...
                target_joint_angles = ikresults[0][:5]
                self.target_gripper_dist = -0.174  # Keep jaw closed until placed

            elif self.state == "releasing_object":
                self.target_gripper_dist = 1.75  # Open jaw to release
        ...
        ## Prepare the action
        joint_action = joint_angles[:5].copy()  # Default action is to maintain current joint angles
        if len(self.planned_joint_actions) == 1: # RRT failed to find a path
            ...
            # print(f"Ave. Joint Angle Diff: {np.mean(np.abs(joint_action - joint_angles[:5])):.4f}")
            if np.all(np.abs(joint_action - joint_angles[:5]) < self.joint_angle_threshold): # Only increment if close enough
