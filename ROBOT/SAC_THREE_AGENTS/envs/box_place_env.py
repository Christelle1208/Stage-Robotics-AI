...
class BoxPlaceEnv(PlaceEnv):
    ...
    # X is randomised left/right each episode; Y stays fixed
    BOX_X_LEFT   = -0.20   # left side  (negative X)
    BOX_X_RIGHT  =  0.20   # right side (positive X)
    BOX_FIXED_Y  =  -0.12   # fixed lateral offset

    # Set at runtime, read by task_manager to sync the reach viewer
    BOX_FIXED_X  =  0.20
    ...
    def _set_box_goal(self):
        """Randomise box left/right in X (or use pre-set BOX_FIXED_X), then set goal_pos above it."""
        # During chained inference task_manager sets BOX_FIXED_X before calling reset.
        # During standalone training we randomise here.
        if not hasattr(self, '_box_x_overridden'):
            gx = self.np_random.choice([self.BOX_X_LEFT, self.BOX_X_RIGHT])
            self.__class__.BOX_FIXED_X = float(gx)
        else:
            gx = self.BOX_FIXED_X
            del self._box_x_overridden   # consume the override flag
        gy = self.BOX_FIXED_Y
        goal_body_pos = self.model.body("goal").pos
        goal_body_pos[0] = float(gx)
        goal_body_pos[1] = gy
        goal_body_pos[2] = 0.0
        _mj.mj_forward(self.model, self.data)
        self.goal_pos = np.array([float(gx), gy, self.release_height], dtype=np.float64)
